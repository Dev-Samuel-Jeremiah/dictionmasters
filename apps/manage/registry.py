...
def assessment_screens(programme, lessons_key, key_prefix, lesson_word):
    ...
    return [
        ...
        {"key": f"{key_prefix}-activity-items", "model": "tricks.LessonActivityItem", "name": "Activity questions",
         ...
         "kind_fields": "item",
         **({"bulk_add": True} if programme == "tricks" else {})},
        ...
        "screens": [
            {"key": "tutor-voices", "model": "tutor.TutorVoice", "name": "Tutor voices",
             ...
             "labels": {"portrait": ("Portrait (optional)",
                        ...
                        "is_default": ("The one new learners start with",
                                       "Only one voice can be the default; choosing another moves it.")}},
    ...
    },
    {
        ...
        "screens": [
            {"key": "radio-programs", "model": "diction_radio.RadioProgram",
             "columns": ["title", "presenter", "order", "is_published"],
             "search": ["title", "tagline", "presenter", "description"], "order": ["order", "title"],
             "form": ["title", "tagline", "description", "presenter", "cover_image", "order", "is_published"],
             "children": ["radio-episodes"],
             "labels": {"order": ("Station order", "Programmes play in this order. Lower numbers play first.")}},
            {"key": "radio-episodes", "model": "diction_radio.RadioEpisode",
             "columns": ["title", "program", "order", "is_published"],
             "search": ["title", "description", "program__title"], "order": ["program__order", "order", "id"],
             "parent": ("program", "radio-programs"),
             "form": ["program", "title", "description", "audio_file", "audio_url", "transcript", "order", "is_published"],
             "labels": {
                 "program": ("Programme", "The show this episode belongs to."),
                 "audio_file": ("Audio file", "Upload MP3, M4A, AAC, WAV, OGG or FLAC audio."),
                 "audio_url": ("Hosted audio URL", "Use this instead of an upload for audio stored on a CDN."),
                 "order": ("Episode order", "Episodes play in this order inside their programme."),
                 "transcript": ("Transcript (optional)", "A written version listeners can read while listening."),
             }},
        ],
    },
    {
        ...
        "screens": [
            {"key": "diction-library-items", "model": "diction_library.LibraryItem",
             ...
             "labels": {
                 ...
                 "is_published": ("Published", "Unpublished items are hidden from users."),
             }},
        ],
    },
    {
...
QUICK_ADDS = [
    ...
    ("radio-episodes", "Radio episode", "🎙️"),
    ("schools", "School", "🏫"),
    ("users", "User", "👤"),
    ("plans", "Billing plan", "💳"),
